...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // 从 URL 获取当前页码、关键词、数据类型等参数
    let currentPage = new URLSearchParams(window.location.search).get('page') || 1; // 获取当前页
    let totalPages = 1;
    let keywords = new URLSearchParams(window.location.search).get('keywords') || '';
    let selectedDataType = new URLSearchParams(window.location.search).get('data_type') || 'All';
    let selectedTask = new URLSearchParams(window.location.search).get('task') || 'All';
    ...
    function displayResults(results) {
        if (results.length > 0) {
            ...
            results.forEach(result => {
                ...
                item.className = 'result-item';
                item.innerHTML = `
                    <p><strong>Dataset Name:</strong> <span>${result['dataset entity']}</span></p>
                    <p><strong>arXiv ID:</strong> <a href="https://arxiv.org/abs/${result['arxiv id']}" target="_blank">${result['arxiv id']}</a></p>
    ...
    // 更新 URL
    function updateURL() {
        const newURL = `${window.location.pathname}?page=${currentPage}&keywords=${encodeURIComponent(keywords)}&data_type=${encodeURIComponent(selectedDataType)}&task=${encodeURIComponent(selectedTask)}`;
        history.pushState(null, '', newURL); // 不刷新页面，更新 URL
    }
